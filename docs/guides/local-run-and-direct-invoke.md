...
```mermaid
flowchart TD
    subgraph pathA[Path A: Full host]
        client[Client] -->|HTTP request| host[func host + route table]
        host --> greetA[greet(req)]
    end

    subgraph pathB[Path B: Direct invoke]
        invoke[invoke.py script] -->|construct HttpRequest| greetB[greet(req)]
        greetB -->|response body/status| invoke
    end
```
...
## Related Patterns
- [MCP Server Example](../patterns/ai-and-agents/mcp-server-example.md)
- [Durable Unit Testing](../patterns/orchestration-and-workflows/durable-unit-testing.md)
- [Durable Entity Counter](../patterns/orchestration-and-workflows/durable-entity-counter.md)
